...
def main():
	...
	train_data[sentence_idx]: the tokenized sentence at idx from EUD
	...
	for i in range(10):
		...
		if "train" in sentence_id:
			...
			sentence = list(train_data[sentence_number])
			# print(sentence)
			assert(sentence[word_index].get('lemma') == word_lemma)

		elif "test" in sentence_id:

			sentence = list(test_data[sentence_number])
			# print(sentence)
			assert(sentence[word_index].get('lemma') == word_lemma)
			...
			sentence = list(dev_data[sentence_number])
			# print(sentence)
			assert(sentence[word_index].get('lemma') == word_lemma)
