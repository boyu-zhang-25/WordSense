...
train_word_idx = train_word_idx[:15000]
...
new_dev_X = new_dev_X[:1500]
new_dev_Y = new_dev_Y[:1500]
new_dev_idx = new_dev_idx[:1500]
...
# 10000 examples
# 22 iterations
trainer._model.load_state_dict(torch.load('word_sense_model_.pth'))
...
for test_idx, test_sen in enumerate(new_test_X):
	...
	if all_senses.get(test_lemma, 'e') == 'e':
		...
		if test_result in correct_super:
			...
			correct_sen_sup.append(test_sen)
			correct_word_sup.append(test_sen[new_test_idx[test_idx]])
			correct_sense_sup.append(test_result)
		else:
			incorrect_sen_sup.append(test_sen)
			incorrect_word_sup.append(test_sen[new_test_idx[test_idx]])
			incorrect_sense_sup.append(test_result)            
		...
		if new_test_Y[test_idx][test_result] == 1:
			correct_count += 1
			correct_sen.append(test_sen)
			correct_word.append(test_sen[new_test_idx[test_idx]])
			correct_sense.append(all_definitions[test_lemma][test_result])
		else:
			incorrect_sen.append(test_sen)
			incorrect_word.append(test_sen[new_test_idx[test_idx]])
			incorrect_sense.append(all_definitions[test_lemma][test_result])            
...
with open('correct_known.csv', mode = 'w') as data_file:    
	writer = csv.writer(data_file, delimiter = ',')
	for idx, row in enumerate(correct_sen):
		writer.writerow(row)
		writer.writerow(correct_word[idx])
		writer.writerow(correct_sense[idx])
		writer.writerow('\n')
	print('done with correct known samples')
	
with open('incorrect_known.csv', mode = 'w') as data_file:    
	writer = csv.writer(data_file, delimiter = ',')
	for idx, row in enumerate(incorrect_sen):
		writer.writerow(row)
		writer.writerow(incorrect_word[idx])
		writer.writerow(incorrect_sense[idx])
		writer.writerow('\n')
	print('done with incorrect known samples')
	
with open('correct_unknown.csv', mode = 'w') as data_file:    
	writer = csv.writer(data_file, delimiter = ',')
	for idx, row in enumerate(correct_sen_sup):
		writer.writerow(row)
		writer.writerow(correct_word_sup[idx])
		writer.writerow(correct_sense_sup[idx])
		writer.writerow('\n')
	print('done with correct unknown samples')
	
with open('incorrect_unknown.csv', mode = 'w') as data_file:    
	writer = csv.writer(data_file, delimiter = ',')
	for idx, row in enumerate(incorrect_sen_sup):
		writer.writerow(row)
		writer.writerow(incorrect_word_sup[idx])
		writer.writerow(incorrect_sense_sup[idx])
		writer.writerow('\n')
	print('done with correct known samples')
